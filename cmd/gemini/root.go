...
func run(cmd *cobra.Command, _ []string) error {
	...
	if len(oracleConfig.Hosts) > 0 {
		oracle, err = drivers.NewCQL(baseCtx, "oracle", schema, oracleConfig, logger.Named("oracle_store"))
		if err != nil {
	...
	test, err := drivers.NewCQL(baseCtx, "test", schema, testConfig, logger.Named("test_store"))
	if err != nil {
		return errors.Wrap(err, "failed to create oracle store")
	}
