...
func TestOutputToFile(t *testing.T) {
	...
	for _, item := range tests {
		t.Run("Compression_"+item.Compression.String(), func(t *testing.T) {
			...
			logger, err := stmtlogger.NewFileLogger(file, item.Compression, zap.NewNop())
			if err != nil {
			...
			data := stmtlogger.Item{
				...
				Host:      "test_host",
				Start:     stmtlogger.Time{time.Now()},
				Type:      typedef.InsertStatementType,
			}
			...
			toCompare := strings.Trim(item.ReadData(t, Must(os.Open(file))), "\n")
...
func BenchmarkLogger(b *testing.B) {
	...
	for _, compression := range runs {
		b.Run(compression.String(), func(b *testing.B) {
			...
			logger := Must(stmtlogger.NewFileLogger(file, compression, zap.NewNop()))
			rows := &atomic.Int64{}
