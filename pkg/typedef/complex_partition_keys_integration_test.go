...
func TestComplexTypesAsPartitionKeys_Integration(t *testing.T) {
	...
	t.Run("FrozenListAsPartitionKey", func(t *testing.T) {
		...
		require.NoError(t, session.Query(createTableQuery).Exec(), "Failed to create table with frozen list partition key")
	...
	t.Run("FrozenSetAsPartitionKey", func(t *testing.T) {
		...
		require.NoError(t, session.Query(createTableQuery).Exec(), "Failed to create table with frozen set partition key")
	...
	t.Run("FrozenMapAsPartitionKey", func(t *testing.T) {
		...
		require.NoError(t, session.Query(createTableQuery).Exec(), "Failed to create table with frozen map partition key")
	...
	t.Run("FrozenTupleAsPartitionKey", func(t *testing.T) {
		...
		require.NoError(t, session.Query(createTableQuery).Exec(), "Failed to create table with frozen tuple partition key")

		// Insert data using tuple as slice
		insertQuery := fmt.Sprintf("INSERT INTO %s.%s (pk, v) VALUES (?, ?)", keyspace, tableName)
		testTuple := []any{1, "test"}
	...
	t.Run("FrozenUDTAsPartitionKey", func(t *testing.T) {
		...
		require.NoError(t, session.Query(createTypeQuery).Exec(), "Failed to create UDT")
